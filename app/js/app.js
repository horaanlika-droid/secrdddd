...
/* ---------- чат поддержки ---------- */
const CHAT_HINTS = ['Мне тяжело', 'Мне тревожно', 'Напомни, кто я', 'Дай совет на сейчас', 'Хочу паузу', 'Побудь рядом'];
const CHAT_TIPS = [
...
function chatReply(text) {
  ...
  if (has('кто я', 'напомни', 'какой я', 'какая я', 'забыл', 'забыла', 'потерял', 'потеряла', 'кто ты')) {
    ...
    reply = rnd([
      ...
      `Спасибо, что пишешь${name}. Нажми любую подсказку ниже — у каждой теперь несколько вариантов ответа, или напиши по-своему.`
    ]);
  }
  return reply;
}
